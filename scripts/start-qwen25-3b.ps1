...
<#
.SYNOPSIS
    agent-test 本地 Qwen2.5-3B-Instruct (Q8_0) 一键启动脚本（llama.cpp CUDA + 3060 6GB GPU）。

.DESCRIPTION
    - 定位 HF_HOME 下 CUDA 版 llama.cpp（默认 E:\hf_home\runtime\llama.cpp-cuda，
      需含 ggml-cuda.dll 与 cudart/cublas 运行库）与 GGUF（默认 E:\hf_home\gguf\qwen2.5-3b-instruct-q8_0.gguf）；
    - 以 OpenAI 兼容服务方式启动 llama-server 到 127.0.0.1:<Port>，默认全量 GPU offload；
    - KV Cache 使用 Q8_0 以在 6GB 显存上留出更长上下文；
    - 日志写入 <repo>/logs/llama-server-qwen2.5-3b-instruct-<Port>.log(.err.log)，PID 写入同名 .pid；
...
.PARAMETER Port
    服务端口，默认 8081。
.PARAMETER ModelPath
    覆盖 GGUF 路径；缺省自动指向 E:\hf_home\gguf\qwen2.5-3b-instruct-q8_0.gguf。
.PARAMETER LlamaDir
    覆盖 llama.cpp CUDA 目录；缺省 $env:HF_HOME\runtime\llama.cpp-cuda。
.PARAMETER CtxSize
    上下文长度（token），默认 16384。
.PARAMETER GpuLayers
...
# ---- 定位 GGUF ----
if (-not $ModelPath) { $ModelPath = Join-Path (Get-HfHome) "gguf\qwen2.5-3b-instruct-q8_0.gguf" }
